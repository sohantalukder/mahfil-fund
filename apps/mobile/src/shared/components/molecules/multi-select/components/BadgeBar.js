...
const BadgeBar = ({ selected, data, maxVisible = MULTI_SELECT_LIST_CONSTANTS.MAX_VISIBLE_BADGES, showAll, onToggleShowAll, onRemove, badgeBaseStyle, badgeStyles, badgeTextStyles, }) => {
    ...
    return (_jsxs(View, { style: [layout.row, layout.wrap, gutters.gap_6], children: [badgesToShow.map((stored, idx) => {
                ...
                return (_jsxs(TouchableOpacity, { style: [badgeBaseStyle, badgeStyles, layout.row, layout.itemsCenter], onPress: () => onRemove(stored), accessibilityRole: "button", accessibilityLabel: `Remove ${String(label)}`, children: [_jsx(Text, { variant: "body3", color: "default", style: badgeTextStyles, children: String(label) }), _jsx(View, { style: gutters.marginLeft_4, children: _jsx(IconByVariant, { path: "delete", width: 12, height: 12, color: colors.text }) })] }, `badge-${String(stored)}-${idx}`));
            }), shouldShowMore && (_jsx(TouchableOpacity, { style: [badgeBaseStyle, badgeStyles, layout.row, layout.itemsCenter, { backgroundColor: colors.primary }], onPress: () => onToggleShowAll(true), accessibilityRole: "button", accessibilityLabel: `Show ${remaining} more items`, children: _jsxs(Text, { variant: "body3", color: "white", style: badgeTextStyles, children: ["+", remaining] }) })), showAll && selected.length > maxVisible && (_jsx(TouchableOpacity, { style: [badgeBaseStyle, badgeStyles, layout.row, layout.itemsCenter, { backgroundColor: colors.primary }], onPress: () => onToggleShowAll(false), accessibilityRole: "button", accessibilityLabel: "Show less items", children: _jsx(Text, { variant: "body3", color: "white", style: badgeTextStyles, children: "Show less" }) }))] }));
};
