...
const AnimatedTextInput = ({ onExpandChange, onSearch, value = '' }) => {
    ...
    const handleDebouncedSearch = useCallback(debounceHandler((text) => {
        onSearch?.(text);
    }, 300), [onSearch]);
    const handleAnimationComplete = useCallback((expanded) => {
    ...
    const containerStyle = useMemo(() => [layout.row, layout.itemsCenter, animatedContainerStyle], [layout.row, layout.itemsCenter, animatedContainerStyle]);
    // Memoize input container style
    const inputContainerStyle = useMemo(() => [gutters.marginLeft_4, animatedInputStyle], [gutters.marginLeft_4, animatedInputStyle]);
    ...
    return (_jsxs(Animated.View, { style: [
            ...
            { minHeight: rs(38) },
            containerStyle,
        ], children: [_jsx(IconButton, { onPress: toggleSearch, icon: isExpanded ? 'leftArrow' : 'search' }), _jsx(Animated.View, { style: [layout.flex_1, inputContainerStyle], children: _jsx(TextInput, { ref: textInputRef, style: textInputStyle, placeholder: "Search...", placeholderTextColor: placeholderColor, onChangeText: handleSearch, returnKeyType: "search", onSubmitEditing: handleTextInputSubmit, onBlur: handleBlur, editable: isExpanded, defaultValue: value, autoCorrect: false, autoCapitalize: "none", spellCheck: false, clearButtonMode: "never" }) })] }));
};
