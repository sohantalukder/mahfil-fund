...
const TextInput = ({ defaultValue = '', label, labelStyle, leftIcon, name, onChangeText, placeholder = '', rightHandler, rightIcon, style = {}, validationRules = undefined, wrapperStyle, errorMessage, required = false, ...props }) => {
    ...
    const getValidationSchema = useCallback((validationRuleKey) => {
        if (validationRuleKey in commonSchemas) {
            return commonSchemas[validationRuleKey];
        }
        return null;
    }, []);
    ...
    // Compute label style
    const computedLabelStyle = useMemo(() => [gutters.paddingBottom_6, labelStyle], [gutters.paddingBottom_6, labelStyle]);
    // Compute error text style
    const errorTextStyle = useMemo(() => [typographies.body2, { color: colors.error, marginTop: 4 }], [typographies.body2, colors.error]);
    /**
    ...
    return (_jsxs(View, { style: [layout.fullWidth, layout.flexShrink_1, wrapperStyle], children: [_jsx(AnimatedLabel, { labelStyle: computedLabelStyle, label: required ? `${label} *` : (label ?? ''), value: currentValue, isFocused: isFocused }), _jsxs(View, { style: containerStyle, children: [leftIcon ? _jsx(View, { children: leftIcon }) : null, _jsx(RNTextInput, { testID: "text-input", numberOfLines: 1, onBlur: handleOnBlur, onChangeText: handleOnChange, onFocus: handleOnFocus, placeholder: label ? '' : placeholder, placeholderTextColor: colors.gray4, ref: inputReference, selectionColor: colors.primary, style: styles.input, value: currentValue, ...props }), rightIcon ? (_jsx(TouchableOpacity, { activeOpacity: 0.5, hitSlop: { bottom: 10, left: 10, right: 10, top: 10 }, onPress: handleRightPress, children: rightIcon })) : null] }), _jsx(InputLabel, { text: error, labelStyle: errorTextStyle })] }));
};
